...
def get_srx_metadata_for_star_results(conn) -> pd.DataFrame:
    """
    Get srx metadata for all samples in the screcounter_star_results table.
    Args:
        conn: psycopg2 connection object
    Returns:
        pd.DataFrame: srx metadata for all samples in the screcounter_star_results table
    """
    query = """
SELECT * FROM "srx_metadata" WHERE "srx_accession" IN (SELECT DISTINCT "sample" FROM "screcounter_star_results")
"""
    return pd.read_sql(query, conn)
